...
function isTodayFolder() {
  DIR_PATH=$1
  if [[ -d "$DIR_PATH" ]]; then
    DIR_CREATION_DATE=$(stat -f %SB -t %Y-%m-%d "$DIR_PATH")
    TODAY_DATE=$(date +%Y-%m-%d)
    if [[ "$DIR_CREATION_DATE" == "$TODAY_DATE" ]]; then
        echo 0 # True - The Folder exists and was created today
    else
       echo 1 # False - The Folder exists and is older then a day
    fi
  else
    echo 1  # False - The Folder does not exist
  fi
}