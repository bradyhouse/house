...
(
    ...
    # Generate the siesta stub file
    cp -rf "tpl/siesta_stub_header" ${siesta_stub_file} || exit 90
    $(echo -en '\n' >> ${siesta_stub_file};) || exit 90
    $(cat "tpl/siesta_stub" >> ${siesta_stub_file};) || exit 90
    $(./house.substr '{{classname}}' ${extjs_class_name} ${siesta_stub_file};) || exit 90
)
#catch
case $? in
    0)  echo "stubbed $1"
        ;;
    86) echo ""
        echo "Siesta Stub Generation Tool"
        echo ""
        echo "Usage:"
        echo ""
        echo "       $0 \"[e]\" \"[s]\""
    ...
    89) echo "FUBAR ~ ExtJS class file is badly formatted or incompatible. Verify that the class is defined using single and not double quotes."
        ;;
    90) echo "FUBAR ~ Failed while trying to write the siesta stub file. Can you say \" Per-mis-sions\"."
        ;;
