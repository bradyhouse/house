...
jasmine.showDebugPrompt = function (callback) {
    if (navigator.userAgent.toLowerCase().match(/chrome|safari|msie/)) {
        ...
        div.innerHTML = [
            ...
            '<button id="house-debug-button">OK</button>',
            '<p></p>'
        ].join("");

        document.body.appendChild(div);

        var button = document.getElementById("house-debug-button");
...
HouseTestRunner = {
    ...
    Reporter: function () {
        ...
        HouseTestRunner.reporter = this;
    }
};


HouseTestRunner.Reporter.prototype = {
    initContextMapping: function () {
//        var xhr = new jasmine.XmlHttpRequest();
//        window.alert(HouseTestRunner.testArtifactServerURL + "context-directory-mapping");
//        xhr.open("GET", HouseTestRunner.testArtifactServerURL + "context-directory-mapping", false);
//        xhr.send(null);
//        if (xhr.status === 200) {
...
//        }
    ...
    checkForCleanup: function (spec) {
        this.checkDom(spec);
        HouseTestRunner.checkGlobals(spec);
    }
};
