<template>
  ...
  import { OutlineFoldNodeInterface } from './OutlineFoldNodeInterface';
  
  withDefaults(
    defineProps<{
      node: OutlineFoldNodeInterface;
      depth?: number;
      className?: (node: OutlineFoldNodeInterface, depth: number) => string;
    }>(),
    { depth: 0 },
  );
  
  defineEmits<{
    (event: 'click-title', node: OutlineFoldNodeInterface): void;
  }>();
