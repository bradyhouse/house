...
const paginatedOptions = computed(() => {
    ...
    return filteredOptions
        .value
        .slice(0, startIndex + limit.value)
...
watch(() => props.loading, (newLoadingValue) => {
    isLoading.value = newLoadingValue
    refreshMessage.value++;
});

watch(() => props.isLastPage, (newFlagValue) => {
    ...
    (newOptions) => {
        if (allOptions.value.length === 0) {
            ...
            const uniqueOptions = newOptions.filter(option =>
                !allOptions.value.some(existing => isEqual(existing, option))
            );
...
async function handleScroll(event) {
    if (event?.target) {
        ...
        if (scrollTop <= 100 && hasPreviousPage.value) {
            ...
            const nextLoader = container.children[container
                ?.children.length - 1];
            if (nextLoader) {
...
const handleKeyDown = (e) => {
    ...
    switch(e.key) {
        case 'ArrowDown':
            ...
            if (lastVisibleIndex >= 0 && document.querySelectorAll('.vs__dropdown-option')[lastVisibleIndex]) {
...
function isPagePopulated(page) {
    ...
    return filteredOptions
        .value
        .slice(startIndex, startIndex + limit.value).length > 1 ? true : false;
}
...
<template>
    ...
    <div class="input-field-select" :class="validationState, { 'icon': icon }">
        <v-select id="selector" ref="selectRef" 
            :disabled="disabled" 
