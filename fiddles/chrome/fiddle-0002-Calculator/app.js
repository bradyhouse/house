(function (app, $, undefined) {
    ...
    app.controller = app.controller || {
        ...
        initOnEmptyBuffer: function (ctrl, value, readout, buffer) {
            ...
            if (me.isOperation(value)) {
                me.zeroActiveNumeralBuffer(me, buffer);
                me.zeroActiveFractionalBuffer(me, buffer);
                me.bufferOperation(buffer, value);
                return me.writeToReadOut(readout, '|')
            }
            // If decimal then zero the Active Numeral Buffer, set readout = 0.|
            if (me.isDecimal(value)) {
                me.zeroActiveNumeralBuffer(me, buffer);
                me.bufferDecimal(me, buffer);
                return me.writeToReadOut(readout, '0.|');
            }
            // If Negate then set the Active Sign Buffer to "-", set readout = -|
            if (me.isNegate(value)) {
                me.bufferNegate(me, buffer);
                return me.writeToReadOut(readout, '-|');
            }
            // If Equal do nothing
            if (me.isEqual(value)) {
            ...
            me.bufferNumeralDigit(ctrl, buffer, value);
            me.writeBufferToReadOut(ctrl, readout, buffer);
        },
