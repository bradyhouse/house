...
![Memorize Emoji](https://i.imgur.com/Bo8Rhhu.png)

These markdown files include an additional `Review Question and Answer` section as well as an `iFlash Card` formatted
text file, which can be imported into the `iFlash Card` App.  For more information regarding these files and the
`iFlash Card` app see the <<Memorization Strategy, Memorization Strategy>> below.
