...
function testReactCreate() {
  ...
  cd "scripts";
  ./fiddle.sh create react "${name}" "ts" || exit 1;
  cd ..;
}
...
function tearDown() {
  if [[ -d "./fiddles/react/dist/fiddle-0000-TemplateTs" ]]
  then
    rm -rf "./fiddles/react/dist/fiddle-0000-TemplateTs" || exit $?;
  fi
  if [[ -d "./fiddles/react/fiddle-0000-TemplateTs" ]]
  then
    rm -rf "./fiddles/react/fiddle-0000-TemplateTs" || exit $?;
  fi
  if [[ -d "./fiddles/react/dist/fiddle-0001-Fork" ]]
  then
    rm -rf "./fiddles/react/dist/fiddle-0001-Fork" || exit $?;
  fi
...
function runTest() {
    case ${__TEST_NUM__} in
        ...
        8)  testReactUpdate || exit $?;
            ;;
        9) testReactUpdateAll || exit $?;
            ;;
        10) testReactList || exit $?;
            ;;
        11) testReactBuild || exit $?;
            ;;
        12) testAll || exit $?;
            ;;
        13) tearDown || exit $?;
